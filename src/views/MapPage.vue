  ...
  <ion-page>
    ...
    <ion-content :scroll-y="false" class="map-content">
      ...
      <ion-action-sheet
        ...
        :header="selectedPin?.profiles?.username + '\'s Note'"
...
const startRouting = (pin) => {
  ...
  routingControl = L.Routing.control({
    waypoints: [
      L.latLng(myCenter.value[0], myCenter.value[1]), // Start at user
      L.latLng(pin.lat, pin.lng)                      // End at Pin
    ],
    // Styling the road path green
    lineOptions: {
      styles: [
        { color: '#1a73e8', opacity: 0.15, weight: 9 }, // Shadow/Outer path
        { color: '#2dd36f', opacity: 0.8, weight: 6 }   // Main Green Road Path
      ],
    ...
    createMarker: () => null // Hide the default 'A' and 'B' markers
  }).addTo(map);
...
const startTracking = async () => {
  ...
  watchId = navigator.geolocation.watchPosition(async (pos) => {
    ...
    await supabase.from('locations').upsert({ user_id: user.id, lat: latitude, lng: longitude, updated_at: new Date() });
  }, null, { enableHighAccuracy: true });
};

const centerOnMe = () => {
  const map = mapInstance.value?.leafletObject;
  if (map) map.flyTo(myCenter.value, 17, { duration: 1.5 });
};
...
/* Hide the routing instruction panel to keep the map clean */
...
.ghost-pin { opacity: 0.6; transform: scale(1.2); filter: grayscale(1); }
...
.avatar-box { width: 45px; height: 45px; background: #2dd36f; border: 3px solid white; border-radius: 15px; display: flex; align-items: center; justify-content: center; font-weight: 800; color: white; box-shadow: 0 5px 15px rgba(0,0,0,0.2); }
